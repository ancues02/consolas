...
void Raycaster::CastRays(float posX, float posY, float angleStart, float FOV, const Map& collisionData) {
	...
	for (int i = 0; i < NUMBER_OF_RAYS; ++i) {
		actualAngle = angle + (i * angIncr);
		CastRay(i, posX, posY, actualAngle, collisionData);
	}
}
...
void Raycaster::CastRay(int index, float posX, float posY, float angle, const Map& collisionData)
{
	...
	float sideDistY;
	if(xMult > 0) 
		sideDistX = (1.0f - (posX - floorf(posX))) * deltaDistX;
	else
		sideDistX = (posX - floorf(posX)) * deltaDistX;
	if(yMult > 0)
		sideDistY = (1.0f - (posY - floorf(posY))) * deltaDistY;
	else
		sideDistY = (posY - floorf(posY)) * deltaDistY;
	...
	if (vertical) {
		...
		float incrY = sqrt(pow(currDist, 2) - pow(catX, 2)) * yMult;
		cposY = posY;
	}
	else {
		float catY = cposY - posY;
		float incrX = sqrt(pow(currDist, 2) - pow(catY, 2)) * xMult;
		cposX = posX;
	}

	_raydata[index].hit = cposX >= 0 && cposX < collisionData.getWidth() && cposY >= 0 && cposY < collisionData.getHeight();
	_raydata[index].posX = posX;
	_raydata[index].posY = posY;
